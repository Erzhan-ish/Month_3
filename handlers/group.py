...
async def ban_user(message: types.Message):
    ...
    if not message.reply_to_message:
        await message.answer("Надо сделать реплай на чьето сообщение")
    else:
...
async def check_bad_group(message: types.Message):
...
async def delete_images(message: types.Message):
    await message.delete()
    await message.answer("Нельзя отправлять картинки и гифки")
